...
class AutocompleteHelper
{
     ...
     * @param \Ivory\GoogleMap\Places\Autocomplete $autocomplete The autocomplete.
     *
     * @throws \Ivory\GoogleMap\Exception\TemplatingException if the autocomplete bound does not have coordinates.
     *
     * @return string The HTML output.
     */
    public function renderJavascripts(Autocomplete $autocomplete)
    {
        ...
        if (!$this->apiHelper->isLoaded() && !$autocomplete->isAsync()) {
            $output[] = $this->apiHelper->render($autocomplete->getLanguage());
        }
        ...
        if ($autocomplete->hasBound()) {
            if (!$autocomplete->getBound()->hasCoordinates()) {
                throw TemplatingException::invalidAutocompleteBound();
            }
        ...
        if (!$this->apiHelper->isLoaded() && $autocomplete->isAsync()) {
            $output[] = $this->apiHelper->render($autocomplete->getLanguage(), 'load_ivory_google_place');
        }
